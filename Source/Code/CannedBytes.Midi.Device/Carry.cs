...
namespace CannedBytes.Midi.Device
{
    ...
    public class Carry
    {
        ...
        public bool IsBitSet(BitFlags bitFlag)
        {
        ...
        /// match with the <see cref="BitFlags"/> property value.</returns>
        public bool AreBitsSet(BitFlags bitFlag)
        {
        ...
        public virtual int ReadFrom(Stream stream, BitFlags bitFlags, out ushort value)
        {
            ...
            else
            {
                ...
                if (newLength == 2)
                {
                    ...
                    if (!IsBitSet(Device.BitFlags.Word))
                    {
                    ...
                    else if (!IsBitSet(Device.BitFlags.HiByte))
                    {
                ...
                {
                    // deal with loByte
                    if (!IsBitSet(Device.BitFlags.LoByte))
                    {
                        value = ReadByte(stream);
                        newFlags = BitFlags.LoByte;
        ...
        {
            if (IsBitSet(bitFlags)) return false;
