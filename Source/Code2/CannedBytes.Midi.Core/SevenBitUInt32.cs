...
public readonly struct SevenBitUInt32 : IConvertible
{
    ...
    {
        var success = ValueParser.TryParseToBytes(s, ByteOrder.LittleEndian, out var bytes);

        if (success)
        {
            value = ByteConverter.FromSevenBitBytesToUInt32(bytes, ByteOrder.LittleEndian);
            return true;
        }
