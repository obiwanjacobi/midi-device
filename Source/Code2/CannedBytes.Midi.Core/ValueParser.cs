...
public static class ValueParser
{
    ...
    public static bool TryParseToBytes(string s, BitOrder ordering, [NotNullWhen(true)] out byte[]? bytes)
    {
        ...
        if (TrySplitOnSeparators(s, out var values))
        {
    ...
    private static bool TryParseToBytes(string[] values, NumberStyles numberStyle, [NotNullWhen(true)] out byte[]? bytes)
    {
        bytes = null;
        byte[] result;
        int index = 0;
    ...
    private static bool TrySplitOnSeparators(string s, [NotNullWhen(true)] out string[]? parts)
    {
        s = s.Trim();
        parts = null;
